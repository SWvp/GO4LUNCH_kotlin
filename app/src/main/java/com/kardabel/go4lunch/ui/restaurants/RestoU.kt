...
import com.kardabel.go4lunch.pojo.*
...
class RestoU constructor(
    ...
    val restaurantsWrapperViewStateMediatorLiveData =
    ...
    private fun combine(
        ...
        if (location != null && userWhoMadeRestaurantChoice != null) {
            if (nearbySearchResults != null && restaurantDetailsResults != null && usersSearch != null && usersSearch.isNotEmpty()) {
                restaurantsWrapperViewStateMediatorLiveData.value = mapUsersSearch(
                    nearbySearchResults,
                    restaurantDetailsResults,
                    location,
                ...
                restaurantsWrapperViewStateMediatorLiveData.value = mapWithoutDetails(
                    location,
                    nearbySearchResults,
                    userWhoMadeRestaurantChoice
                )
            } else if (restaurantDetailsResults != null && nearbySearchResults != null) {
                restaurantsWrapperViewStateMediatorLiveData.value = mapWithDetails(
                    location,
                    nearbySearchResults,
                    restaurantDetailsResults,
                    userWhoMadeRestaurantChoice
    ...
    private fun mapWithDetails(
        ...
        for (restaurant in nearbySearchResults.results!!) {
            for (i in 1..restaurantDetailsResults.size) {
    ...
    private fun photoReference(photoList: List<Photo>?): String {
        ...
        if (photoList != null) {
            for (photoReference in photoList) when {
                photoReference.photoReference?.isNotEmpty() == true -> {
